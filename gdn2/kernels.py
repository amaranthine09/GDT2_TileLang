"""TileLang kernels for the chunkwise Gated DeltaNet-2 forward pass.

Six kernels form the training-time pipeline; a seventh handles single-token
...
def cumsum_kernel(
    ...
    @T.prim_func
    def kernel(
        g: T.Tensor(shape, dtype=gate_dtype), #type:ignore
        G: T.Tensor(shape, dtype=gate_dtype), #type:ignore
    ):
...
def intra_diag_kernel(
    ...
    *sub-chunk* and reduces over the channel axis directly, so the decay enters
    ...
    @T.prim_func
    def kernel(
        q: T.Tensor(qk_shape, dtype=input_dtype), #type:ignore
        k: T.Tensor(qk_shape, dtype=input_dtype), #type:ignore
        b: T.Tensor(qk_shape, dtype=input_dtype), #type:ignore
        G: T.Tensor(qk_shape, dtype=gate_dtype), #type:ignore
...
def solve_kernel(
    ...
    def kernel(
        Akk: T.Tensor(a_shape, dtype=accum_dtype), #type:ignore
        Ai: T.Tensor(a_shape, dtype=input_dtype), #type:ignore
    ):
        with T.Kernel(T.ceildiv(S, BS), B * H, threads=threads) as (i_s, i_bh):
            i_b, i_h = i_bh // H, i_bh % H
            t0 = i_s * BS
...
def state_kernel(
    ...
    def kernel(
        ...
        U: T.Tensor(v_shape, dtype=input_dtype), #type:ignore
        G: T.Tensor(k_shape, dtype=gate_dtype), #type:ignore
        initial_state: T.Tensor(st_shape, dtype=state_dtype), #type:ignore
        h: T.Tensor(h_shape, dtype=input_dtype), #type:ignore
        Un: T.Tensor(v_shape, dtype=input_dtype), #type:ignore
        final_state: T.Tensor(st_shape, dtype=state_dtype), #type:ignore
    ):
...
def output_kernel(
    ...
    def kernel(
        ...
        h: T.Tensor(h_shape, dtype=input_dtype), #type:ignore
        Un: T.Tensor(v_shape, dtype=input_dtype), #type:ignore
        o: T.Tensor(v_shape, dtype=output_dtype), #type:ignore
    ):
...
def decode_step_kernel(
    ...
    def kernel(
        ...
        b: T.Tensor(k_shape, dtype=input_dtype), #type:ignore
        w: T.Tensor(v_shape, dtype=input_dtype), #type:ignore
        state: T.Tensor(st_shape, dtype=state_dtype), #type:ignore
        o: T.Tensor(v_shape, dtype=output_dtype), #type:ignore
    ):
        with T.Kernel(T.ceildiv(DV, block_DV), B * H, threads=threads) as (i_v, i_bh):
            i_b, i_h = i_bh // H, i_bh % H
